...
export async function costRoutes(app: FastifyInstance, db: Database.Database): Promise<void> {
  ...
  app.get('/api/costs/summary', async () => {
    ...
    const today = db.prepare('SELECT COALESCE(SUM(cost_usd), 0) as total, COALESCE(SUM(input_tokens + output_tokens + thinking_tokens), 0) as tokens FROM session_costs WHERE created_at >= ?').get(todayStart) as { total: number; tokens: number };
    const week = db.prepare('SELECT COALESCE(SUM(cost_usd), 0) as total FROM session_costs WHERE created_at >= ?').get(weekStart) as { total: number };
    const month = db.prepare('SELECT COALESCE(SUM(cost_usd), 0) as total FROM session_costs WHERE created_at >= ?').get(monthStart) as { total: number };
  ...
  app.get('/api/costs/budget', async () => {
    const config = db.prepare("SELECT * FROM budget_config WHERE id = 'default'").get();
  ...
  app.put('/api/costs/budget', async (req) => {
    ...
    `).run(body.perSessionMaxUsd ?? null, body.perMachineDailyUsd ?? null, body.globalDailyUsd ?? null, body.enabled ? 1 : 0);
    return { updated: true };
  });
}
